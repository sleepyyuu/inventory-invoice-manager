...
export default function Invoices(props) {
  ...
  const [newInvoiceBuyerIndex, setNewInvoiceBuyerIndex] = useState("");
  ...
  const handlePostAction = async (e) => {
    ...
    let body = {
      ...
      buyer_name: buyers[newInvoiceBuyerIndex].company_name,
    };
    if (newInvoiceDetails !== "") {
      body.details = newInvoiceDetails;
    }
    if (menuStateCreate) {
      body.buyer = buyers[newInvoiceBuyerIndex]._id;
  ...
  const handleEdit = async (invoice) => {
    ...
    for (let i = 0; i < invoices; i++) {
      console.log(invoices[0]._id);
      if (invoice._id === invoices[i]._id) {
        index = i;
        break;
      }
    }
              ...
              <form>
                <div>
                  ...
                  <select
                    ...
                    name="invoiceBuyer"
                    onChange={(e) => {
                      setNewInvoiceBuyerIndex(e.target.value);
                      setNewInvoiceBuyer(buyers[e.target.value]._id);
                    }}
                    value={newInvoiceBuyerIndex}
                  >
                    {buyers.map((buyer, counter) => {
                      return (
                        <option value={counter} key={uniqid()}>
                          {buyer.company_name}
                        </option>
                      );
                    })}
