...
export default function Products() {
  ...
  const getInitialDB = async () => {
    const [responseBuyers, responseProducts] = await Promise.all([verify("readAll", "/buyers"), verify("readAll", "/products")]).catch(
      function (err) {
        console.log(err);
      }
    );
    setBuyers(values.responseBuyers);
    setProducts(values.responseProducts);
    setLoading(false);
  };
  const getDB = async () => {
