...
function App() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLoadingProgress((loadingProgress) => {
        console.log(loadingProgress + Math.floor(Math.random() * 20));
  ...
  return (
    <BrowserRouter>
      <Routes>
        <Route path="/" element={<LoadingBar loadingProgress={loadingProgress} style={{ width: `${loadingProgress}vw` }}></LoadingBar>}>
