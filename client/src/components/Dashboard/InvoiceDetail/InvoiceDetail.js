...
export default function InvoiceDetail(props) {
  ...
  return loading ? null : (
    <div id="invoiceDetailPageContainer">
      <div id="invoiceDetailPage">
                ...
                {invoice.product.map((prod) => {
                  return (
                    <tr key={uniqid()} id="printInvoiceBodytr">
                      ...
                      <td className="priceDescription">
                        ${(prod.quantity * prod.price - prod.quantity * prod.discountPerUnit).toFixed(2)}
                      </td>
                    </tr>
                  );
                })}
